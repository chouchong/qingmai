   ...
   <body class='wst-page'>
       ...
       <form method='post' action='<?php echo U("Admin/Ads/index");?>' autocomplete="off">
		  ...
		  广告位置：<select name="adPositionId">
                ...
                <option value='-2' <?php if($params['adPositionId'] == -2 ): ?>selected<?php endif; ?>>手机首页广告</option>
                <!-- <option value='-3' <?php if($params['adPositionId'] == -3 ): ?>selected<?php endif; ?>></option> -->
             </select>
  		...
  		<button type="submit" class="btn btn-primary glyphicon glyphicon-search">查询</button> 
	       ...
	       <?php if(in_array('gggl_01',$WST_STAFF['grant'])){ ?>
        ...
        <table class="table table-hover table-striped table-bordered wst-list">
           ...
           <tbody>
            <?php if(is_array($Page['root'])): $i = 0; $__LIST__ = $Page['root'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
               ...
               <?php elseif($vo['adPositionId'] == -2 ): ?>手机首页广告
               <?php elseif($vo['adPositionId'] == -3 ): ?>
