...
class IndexModel extends BaseModel {
	...
	public function saveConfigsForCode(){
		...
		if(!empty($rs)){
			$m = M('sys_configs');
			foreach ($rs as $key => $v){
				$result = $m-> where('fieldCode="'.$v['fieldCode'].'"')->setField('fieldValue',I($v['fieldCode'],'',false));
				if(false === $result){
				    $rd['status']= -1;
				}
			}
	...
	public function getWeekInfo(){
		...
		//新增门票
		$ret['goodsNew'] = M('goods')->where('createTime>"'.$weekDate.'"')->count();
		//新增订单
		$ret['ordersNew'] = M('orders')->where('orderStatus >=0 and createTime>"'.$weekDate.'"')->count();
		return $ret;
	}
	...
	public function getSumInfo(){
		...
		//订单
		$ret['ordersSum'] = M('orders')->where('orderStatus >=0')->count();
		//订单总金额
		$ret['moneySum'] = M('orders')->where('orderStatus >=0')->sum('totalMoney');
		return $ret;
	}
}