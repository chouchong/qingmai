...
function delGoods(id){
	layer.confirm("您确定要删除该门票？",{icon: 3, title:'系统提示'},function(tips){
...
function batchDel(){
	layer.confirm("您确定要删除这些门票？",{icon: 3, title:'系统提示'},function(){
	      var ids = getChks();
...
function sale(v){
	var ids = getChks();
	if(ids==''){
		WST.msg('请先选择门票!', {icon: 5});
		return;
	}
	layer.confirm((v==1)?"您确定要上架这些门票？":"您确定要下架这些门票？",{icon: 3, title:'系统提示'},function(tips){
	    var loading = layer.load('正在处理，请稍后...', 3);
	    layer.close(tips);
	    var params = {};
	    params.ids = ids;
	    ...
	    $.post(Think.U('Home/Goods/sale'),params,function(data,textStatus){
	    	...
	    	if(json.status=='1'){
	    		...
	    		WST.msg('上架失败，请核对门票信息是否完整!', {icon: 5});
	    	}else if(json.status=='2'){
	    		WST.msg('已成功上架门票'+json.num+" 件，请核对未能上架的门票信息是否完整。", {icon: 5},function(){
	    			location.reload();
	    		});
	    	}else if(json.status=='-3'){
	    		WST.msg('上架门票失败!您的店铺权限不能出售门票，如有疑问请与商城管理员联系。', {icon: 5,time:3000});
	    	}else{
	    		WST.msg('操作失败!', {icon: 5});
	    	}
	    });
...
function editGoods(menuId){
	  ...
	  //  $.post(Think.U('Home/Goods/edit'),params,function(data,textStatus){
		 //   layer.close(loading);
			...
			// }else if(json.status=='-3'){
			// 	if(params.isSale==1){
			// 		WST.msg('您的店铺权限不能上架门票，所编辑门票已被存放在仓库中，如有疑问请与商城管理员联系!', {icon: 5,time:4000},function(){
			// 			if((menuId=='toEditGoods')){
			// 				location.href= Think.U('Home/Goods/toEdit','umark=toEditGoods');
			// 			}else{
			// 				location.href=Think.U('Home/Goods/'+menuId);
...
function editGoodsCat(){
	   ...
	   var loading = layer.load('正在提交门票分类信息，请稍后...', 3);
...
function delGoodsCat(id){
	layer.confirm("您确定要删除该门票分类吗？",{icon: 3, title:'系统提示'},function(tips){
		layer.load('正在处理，请稍后...', 3);
		layer.close(tips);
...
function shopOrderProduce(id){
	layer.confirm('您确定打包该门票吗？',{icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
...
function batchShopOrderProduce(){
	...
	layer.confirm('您确定打包这些门票吗？',{icon: 3, title:'系统提示'}, function(tips){
	    var ll = layer.load('数据处理中，请稍候...');
...
function delAttrCat(id){
	layer.confirm("您确定要删除该门票类型及其下的属性吗？",{icon: 3, title:'系统提示'},function(tips){
	    var loading = layer.load('正在处理，请稍后...', 3);
	    layer.close(tips);
	    var params = {};
...
function toEditGoodsBase(fv,goodsId,flag){	
	if((fv==2 || fv==3) && flag==1){
		WST.msg('该门票存在门票属性，不能直接修改，请进入编辑页修改', {icon: 5});
		return;
	}else{
