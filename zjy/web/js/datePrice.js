...
$(function() {
    init();
    $.ajax({
        url: "http://192.168.0.125/Api/Api/getDtp",
...
function addDay(date, price, tickets, id) {
    ...
    if (tickets == '0') {
        htmls = htmls + '<div class="col col-14"><p class="dd" id="' + day[2] + '">' + day[2] +
            '</p><p class="price">' + parseInt(price) +
            '</p><p class="ticket">售罄</p></div>';
    } else if(isbgeinSell){
                htmls = htmls + '<div class="col col-14"><p class="dd"  id="' + day[2] + '">' + day[2] +
            '</p><p class="price">' + parseInt(price) +
            '</p><p id="ticket">已售</p><span id="'+id+'"></span></div>';
    } else {
        htmls = htmls + '<div class="col col-14"><p  class="dd"  id="' + day[2] + '">' + day[2] +
            '</p><p class="price">' + parseInt(price) +
            '</p><p class="ticket">余' + tickets +
            '</p><span id="'+id+'"></span></div>';
    }
}

function addPastDay(date, price) {
    var day = date.split('-');
    htmls = htmls + '<div class="col col-14 disabled"><p class="dd"  id="' + day[2] + '">' + day[2] +
...
function addAllDays() {
    ...
    for (var i = 0; i < allMonths.length; i++) {
        for (var j = 0; j < allMonths[i].empday; j++) {
            dateAll.push({ "timeId": "", "drivesStock": "", "adultPrice": "", "daydata": "", "drivesType": "", "timeDesc": "" });
            index++;
        }
