    ...
    getRight: function () {
        ...
        for (var i = 1; i <= 42; i++) {
            ...
            if ((i - 1) >= week && (i - c) <= days) {
                ...
                var priceNum = commonUtil.getNum((i - c));
                var timeId = commonUtil.getid((i - c));
                var priceStr = "";
                var classStyle = "";
                if (price != -1) {
                    priceStr = "<dfn>"+priceNum+"</dfn>&nbsp;&nbsp;<dfn>¥</dfn>" + price;
                    classStyle = "class='on'";
                }
				...
				if(obj.year==new Date().getFullYear()&&obj.month==new Date().getMonth()+1&&i-c==new Date().getDate()){
					html += '<td  ' + classStyle + ' date="' + timeId + '" price="' + price + '"><a><span class="date basefix" style="color:red">今天</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
				}
				else{
                	html += '<td  ' + classStyle + ' date="' + timeId + '" price="' + price + '"><a><span class="date basefix">' + (i - c) + '</span><span class="team basefix" style="display: none;">&nbsp;</span><span class="calendar_price01">' + priceStr + '</span></a></td>';
				}
                if (index == 6) {
...
var commonUtil = {
    getPrice: function (day) {
        ...
        if(obj.priceArr != null){
            for (var i = 0; i < obj.priceArr.length; i++) {
            if (obj.priceArr[i].daydata == dt) {
                return obj.priceArr[i].adultPrice.split('.')[0];
                }
            }
        }
        return -1;
    ...
    getNum: function (day) {
        ...
        if(obj.priceArr != null){
            for (var i = 0; i < obj.priceArr.length; i++) {
                if (obj.priceArr[i].daydata == dt) {
                    // console.log(obj.priceArr[i].drivesStock);
                    if(obj.priceArr[i].drivesStock>0){
                        return obj.priceArr[i].drivesStock;
                    }
                    return 0;
                }
            }
    ...
    getid: function (day) {
        ...
        if(obj.priceArr != null){
            for (var i = 0; i < obj.priceArr.length; i++) {
                if (obj.priceArr[i].daydata == dt) {
                    // console.log(obj.priceArr[i].drivesStock);
                    if(obj.priceArr[i].timeId>0){
                        return obj.priceArr[i].timeId;
                    }
                    return 0;
                }
            }
    ...
    chooseClick: function (sender) {
        ...
        if (el != null) {
            el.value = date;
            Plugins.confirm({title:'信息提示',content:'您确定要删除?',okText:'确定',cancelText:'取消',okFun:function(){
                $.ajax({
                  url: pickerEvent.getTPUrl(),
                  type: "POST",
                  async:false,
                  data:{timeId:date},
                  dataType:'Json',
